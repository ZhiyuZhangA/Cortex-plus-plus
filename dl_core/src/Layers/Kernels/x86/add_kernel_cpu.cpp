...
#define BLOCK_SIZE 8
    ...
    void add_scalar_avx256(float* a, const float* b, float* c, const int n) {
    ...
    void add_kernel_cpu(const Tensor& a, const Tensor& b, const Tensor& result) {
        if (b.size() == 1) {
            add_scalar_avx256(a.ptr<f32_t>(), b.ptr<f32_t>(), result.ptr<f32_t>(), a.size());
        }
        else if (a.size() == 1) {
            add_scalar_avx256(b.ptr<f32_t>(), a.ptr<f32_t>(), result.ptr<f32_t>(), b.size());
        }
        else if (a.size() == b.size()) {
            add_vec_avx256(a.ptr<f32_t>(), b.ptr<f32_t>(), result.ptr<f32_t>(), a.size());
        }
        else {
            throw std::runtime_error("add_kernel_cpu: size mismatch");
        }
    }
}
